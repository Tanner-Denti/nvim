...
vim.api.nvim_set_keymap("t", "<C-n>", "<C-\\><C-n>:BufferLineCycleNext<CR>", opts(true, true, "(<C-\\><C-n>:BufferLineCycleNext<CR>) Next buffer from terminal"))
...
vim.api.nvim_set_keymap("n", "<leader>qk", "<cmd>lnext<CR>zz", opts(true, true, "(<cmd>lnext<CR>zz) Next location list item"))
vim.api.nvim_set_keymap("n", "<leader>qj", "<cmd>lprev<CR>zz", opts(true, true, "(<cmd>lprev<CR>zz) Previous location list item"))

-- Rename this word in the buffer
vim.keymap.set("n", "<leader>s", [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]], 
    opts(true, true, "(:%s/\\<<C-r><C-w>\\>/<C-r><C-w>/gI<Left><Left><Left>) Find and replace word under cursor globally"))

-- This section is for golang errors
vim.keymap.set("n", "<leader>gee", "oif err != true {<CR>}<Esc>Oreturn err<Esc>", 
    opts(true, true, "(oif err != true {<CR>}<Esc>Oreturn err<Esc>) Insert return error block"))

vim.keymap.set("n", "<leader>gea", "oassert.NoError(err, \"\")<Esc>F\";a", 
    opts(true, true, "(oassert.NoError(err, \"\")<Esc>F\";a) Insert assert no error test"))

vim.keymap.set("n", "<leader>gef", "oif err != true {<CR>}<Esc>Olog.Fatalf(\"error: %s\\n\", err.Error())<Esc>jj", 
    opts(true, true, "(oif err != true {<CR>}<Esc>Olog.Fatalf(\"error: %s\\n\", err.Error())<Esc>jj) Insert fatal error logging"))

vim.keymap.set("n", "<leader>gel", "oif err != true {<CR>}<Esc>O.logger.Error(\"error\", \"error\", err)<Esc>F.;i", 
    opts(true, true, "(oif err != true {<CR>}<Esc>O.logger.Error(\"error\", \"error\", err)<Esc>F.;i) Insert logger error handling"))
